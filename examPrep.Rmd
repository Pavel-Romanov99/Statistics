...
```{r}
#нулевата хипотеза е че имат равни средни трите извадки от данни
#правим дата фрейм с данните от задачата
  exams_df <- data.frame(
...
examinor3 = c(4 ,6 ,4 ,2 ,4 ,5 ,5 ,3 ,6 ,4)
 ...
 stacked_exam_df <- stack(exams_df)
 
#гледаме дали са нормално разпределени данните
 
qqnorm(exams_df$examinor1)
qqline(exams_df$examinor1)
shapiro.test(exams_df$examinor1)

qqnorm(exams_df$examinor2)
qqline(exams_df$examinor2)
shapiro.test(exams_df$examinor2)
 
qqnorm(exams_df$examinor3)
qqline(exams_df$examinor3)
shapiro.test(exams_df$examinor3)
 
#и трите са нормално  разпределени тоест можем да направим тест дали имат еднакво средно ако са норм разпр

oneway.test(values ~ ind, data = stacked_exam_df)

#Не можем да отхвърлим хипотезата че имат равни средни
#друг начин да се провери съшата хипотеза

anova(lm(values ~ ind, data = stacked_exam_df))
```
...
#3.взимаме данните от файла
drug_df <- read.csv("./data.txt")

#тъй като имаме сдвоени данни, т.е даваме лекартво на един и същ пациент ползваме aov
aov(response ~ drug + Error(patient), data = drug_df) %>% summary()

#не можем да отхвърлим хопотезата, че имаме лекарствата действат еднакво
